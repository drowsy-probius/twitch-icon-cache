...
export const CACHE_TIME: string =
  process.env.TWITCH_ICON_CACHE_CACHE_TIME || '1d';

// `/refresh/:streamer?key=` api에 접속할 때 사용할 키 값. 외부 유출에 유의
export const REFRESH_KEY: string =
  process.env.TWITCH_ICON_REFRESH_KEY || 'secretkey';
...
// `/icon`, `/image` api에서 제공하는 아이콘 이미지 크기 목록
export const ICON_SIZE: number[] = [32, 100];
