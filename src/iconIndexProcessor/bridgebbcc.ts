...
import { saveIcon, fetchImageAsBuffer, isImageInLocal } from "./functions";
...
class BridgeBBCC {
  ...
  async resolveAbsoluteIconUrl(
    ...
    if (this.hint.found) {
      ...
      if (path.startsWith("http://") || path.startsWith("https://"))
        return path;
      return new URL(icon[this.hint.propsName] as string, this.hint.baseUrl)
        .href;
    }
    ...
    const path2 = new URL(`images/dccon/${icon.name}`, this.hint.domainUrl)
      .href;
  ...
  async formatIcon(
    ...
    const originIconUrl = await retry(
      () => this.resolveAbsoluteIconUrl(streamer, icon),
      {
        retries: 5,
      }
    );
  ...
  async formatIconIndex(
    ...
    const iconData = await Promise.all(
      jsonData.dcConsData.map(async (icon: IconBridgeBBCC): Promise<Icon> => {
        return this.formatIcon(streamer, icon).then(async (icon: Icon) => {
          const imageBuffer = await retry(
            () => fetchImageAsBuffer(icon.originPath, this.logger),
            {
              retries: 5,
            }
          );
          icon.iconHash = createHash("sha256")
            .update(imageBuffer)
            .digest("hex");
          ...
          return retry(
            () => saveIcon(imageBuffer, icon, streamer.name, this.logger),
            {
              retries: 5,
            }
          );
