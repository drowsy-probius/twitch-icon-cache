...
export default class IconIndexProcessor {
  ...
  downloadIndexFromUrl() {
    if (this.streamer.type === 0)
      return new OpenDccon(this.logger).downloadIndexFromUrl;
    if (this.streamer.type === 1)
      return new BridgeBBCC(this.logger).downloadIndexFromUrl;
  ...
  async saveToDatabase(iconIndex: IconIndex) {
    ...
    try {
      if (streamerDoc === null) {
        throw new Error(
          `${this.streamer.name} is not exists in streamerList database.`
        );
      }

      const icons = await Promise.all(
        iconIndex.icons.map(async (icon: Icon) => {
          const iconDoc = await IconListModel.findOne({
      ...
      this.logger.info(
        `[saveToDatabase] Total ${icons.length} icons are inserted to database`
      );
    } catch (err: any) {
      if (err.code === 11000) {
        // dup key error
        ...
        const icons: Icon[] = [];
        const errorIconObjectIds = err.result.result.writeErrors.map(
          (error: any) => {
        ...
        await IconInfoListModel.deleteMany({
          icon: { $in: errorIconObjectIds },
        });
        // 3- adding the orders
        await IconInfoListModel.insertMany(icons);
      } else {
  ...
  async run() {
    try {
      if (this.streamer.url === undefined) {
        throw new Error(
          `url is undefined: ${JSON.stringify(this.streamer, null, 2)}`
        );
      }

      if (this.streamer.type === 0) {
        this.logger.info(
          `[IconProcessor] Processing icons start! ${JSON.stringify(
            this.streamer,
            null,
            2
          )}`
        ...
        processor
          .downloadIndexFromUrl(this.streamer.url)
          .then((iconIndexRaw: IconIndexOpenDccon) =>
            processor.formatIconIndex(this.streamer, iconIndexRaw)
          )
          .then((iconIndex: IconIndex) => this.saveToDatabase(iconIndex))
          .then(() => {
            this.logger.info(
              `[IconProcessor] Processing icons done! ${JSON.stringify(
                this.streamer,
                null,
                2
              )}`
        ...
        throw new Error(
          `Unknown type: ${JSON.stringify(this.streamer, null, 2)}`
        );
      }
    } catch (err) {
