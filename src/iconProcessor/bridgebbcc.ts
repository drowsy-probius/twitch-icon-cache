...
export const processor = (streamer: StreamerData, jsonData: IconIndexBridgeBBCC): Promise<void> => {
  ...
  const findIconUri = async (icon: IconBridgeBBCC): Promise<string> => {
    ...
    const candidateProps = ["uri", "path", "url"];
  ...
  return new Promise(async (resolve, reject) => {
    try
    {
       ...
       */
      const newIconsData = await Promise.all(jsonData.dcConsData.map(async (icon: IconBridgeBBCC, index, arr): Promise<Icon> => {
        /**
