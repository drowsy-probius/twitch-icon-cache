...
const handler = async (req: Request, res: Response) => {
  ...
  const paramSize =
    'small' in req.query
      ? 32
      : ICON_SIZE.includes(Number(req.query['size']))
   ...
   */
  const basePath = isParamSizeExists
    ? getImageBasePath(streamer)
    : getResizeBasePath(streamer, paramSize);
   ...
   */
  const failedListFile = resolve(
    join(getImageBasePath(streamer), FAILED_LIST_FILE)
  );
